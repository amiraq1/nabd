...
def _plan_organize_folder(intent: ParsedIntent, settings: dict) -> ExecutionPlan:
    ...
    if not directory:
        raise ValidationError("Please specify a directory to organize, e.g. /sdcard/Download")
    action = ToolAction(
        tool_name="files",
    ...
    return ExecutionPlan(
        ...
        dry_run=True,
        actions=[action],
        preview_summary=f"Organize files in '{directory}' into category subfolders (images, videos, documents, etc.)",
    )
...
def _plan_backup_folder(intent: ParsedIntent, settings: dict) -> ExecutionPlan:
    ...
    if not source:
        raise ValidationError("Please specify the source folder to back up.")
    if not target:
        raise ValidationError("Please specify the target (destination) directory for the backup.")
    action = ToolAction(
        tool_name="backup",
...
def _plan_convert_video(intent: ParsedIntent, settings: dict) -> ExecutionPlan:
    source = intent.source_path
    if not source:
        raise ValidationError("Please specify the video file to convert.")
...
def _plan_rename_files(intent: ParsedIntent, settings: dict) -> ExecutionPlan:
    directory = intent.source_path
    if not directory:
        raise ValidationError("Please specify the directory whose files should be renamed.")
    prefix = intent.options.get("prefix", "")
    suffix = intent.options.get("suffix", "")
    if not prefix and not suffix:
        raise ValidationError("Please specify a prefix or suffix for renaming.")
    action = ToolAction(
        tool_name="files",
...
def _plan_move_files(intent: ParsedIntent, settings: dict) -> ExecutionPlan:
    ...
    target = intent.target_path
    if not source:
        raise ValidationError("Please specify the file or folder to move.")
    if not target:
        raise ValidationError("Please specify the destination directory.")
    action = ToolAction(
        tool_name="files",
